...
public class GetPhonemeByIdUseCase {
    private final PhonemeRepository repository;

    public GetPhonemeByIdUseCase(PhonemeRepository repository) {
        this.repository = repository;
    }

    @Transactional(readOnly = true)
    public Phoneme execute(PhonemeId id) throws PhonemeNotFoundException {
        return repository.findById(id).orElseThrow(() -> new PhonemeNotFoundException(id));
    }
}
