...
class GetPhonemeByIdController {
    ...
    record PhonemeDetailResponse(String id, String symbol, String name, String category,
    ...
    ResponseEntity<PhonemeDetailResponse> handle(@PathVariable UUID phonemeId)
            throws PhonemeNotFoundException {
        Phoneme p = useCase.execute(new PhonemeId(phonemeId));
        return ResponseEntity.ok(new PhonemeDetailResponse(
