...
class JpaUserPhonemeProgressRepositoryAdapter implements UserPhonemeProgressRepository {
    ...
    @Override
    public Optional<UserPhonemeProgress> findByUserAndPhonemeAndPhrase(
            UserProfileId userId, PhonemeId phonemeId, PhonemePracticePhraseId phraseId) {
    ...
    public List<UserPhonemeProgress> findByUserAndPhoneme(UserProfileId userId, PhonemeId phonemeId) {
        return jpaRepository.findByUserIdAndPhonemeId(userId.value(), phonemeId.value()).stream()
                .map(UserPhonemeProgressEntity::toAggregate).toList();
    }

    @Override
    public UserPhonemeProgress save(UserPhonemeProgress progress) {
        boolean exists = jpaRepository.existsById(progress.id().value());
        UserPhonemeProgressEntity entity = UserPhonemeProgressEntity.fromAggregate(progress, !exists);
